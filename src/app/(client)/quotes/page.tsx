...
export default async function QuotesPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-5 animate-fade-up">
      <div>
        <h1 className="text-lg font-semibold text-[#f1f5f9]">My Quotes</h1>
        <p className="text-sm text-[#94a3b8] mt-0.5">Quotes received for your RFQs</p>
      </div>
      ...
      {quoteList.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-[rgba(148,163,184,0.15)] bg-[#253448]/30 py-16 text-center">
          ...
          <p className="text-sm text-[#64748b] max-w-xs">
            Submit an RFQ and our team will send you competitive quotes within 48 hours.
          </p>
        </div>
      ) : (
        <div className="space-y-2">
          {quoteList.map((quote) => (
            <Link key={quote.id} href={`/rfq/${quote.rfqId}`}>
